 ...
 const Company = require('../models/company_register'); //import comapny_register  model schema
...
//      const  select = '_id username email';
                ...
                if(err) {
                   ...
                   let validatePassword = user.comparePassword(req.body.password);
                     if(!validatePassword) {
                       ...
                       res.send(user);
                       res.json({ success: true, token: token, user: { username: user.username, email: user.email, isadmin: user.is_admin, loggedId: user._id } });
                     }
                 } else {
   ...
   router.get('/profile', (req, res) => {
     User.findOne({_id: req.decoded.userId}).select('username email is_admin').exec((err, user ) =>{
       if (err) {
         res.json({ success: false, message: err });
       } else {
